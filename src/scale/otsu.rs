//! # Otsu thing
//!
//! Some bullshit generated by AI. I don't know how it works and I don't like it. Will be removed in future.

use std::marker::PhantomData;

use crate::{scale::scale_guesser::ScaleGuesser, types::Image};
...
pub fn detect_scale_factor<I: Image>(img: &I) -> f64 {
    ...
    let score_scale = |s: f64| -> f64 {
        let positions: Vec<f64> = (1..((width as f64 / s) as u32))
            .map(|x| x as f64 * s)
            .collect();
        if positions.is_empty() {
            ...
            positions
                .iter()
                .map(|p| interpolate(*p) - baseline)
                .sum::<f64>()
                / positions.len() as f64
        }
    };

    let best_coarse = (15..251)
        .map(|x| x as f64 / 10.0)
        .max_by(|&a, &b| score_scale(a).total_cmp(&score_scale(b)))
        .unwrap_or(0.0);

    (-20..21)
        .map(|x| best_coarse + x as f64 / 100.0)
        .filter(|&x| x > 1.0)
        .max_by(|&a, &b| score_scale(a).total_cmp(&score_scale(b)))
        .unwrap_or(0.0)
}
...
impl OtsuGuesser {
    pub fn new() -> Self {
        OtsuGuesser { _private: PhantomData }
    }
}
